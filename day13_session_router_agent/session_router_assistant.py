...
def main() -> None:
    ...
    tools = [
        {
            ...
            "function": {
                ...
                "description": "在本地知识库中检索与 session 路由、状态概览、多会话选择相关的文本块",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "query": {
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是一位会管理多个学习 session 并进行路由选择的助理 Agent。"
                    "如果需要知识库支持，请先调用工具检索。"
                ),
            },
            {
                ...
                "content": (
                    ...
                    "请先获取必要知识，再判断现在最应该继续推进哪个 session：\n\n"
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是一位会在多个学习 session 之间做优先级路由的助理 Agent。"
            ...
            },
            {
                "role": "user",
                "content": (
                    "请从这些 session 概览中选出当前最应该继续推进的一个，并输出 JSON。\n\n"
                    "字段要求：\n"
                    '- selected_session_id: 字符串，表示选中的 session_id\n'
                    '- reason: 字符串，表示为什么优先选它\n'
                    '- suggested_focus: 字符串，表示这次进入该 session 后最该关注什么\n\n'
    ...
    routing_plan = json.loads(
        clean_json_text(routing_response.choices[0].message.content or "")
    )
    ...
    selected_session_id = routing_plan["selected_session_id"]
    learner_state = load_session_state(selected_session_id)

    print("Selected learner state before update:")
    ...
    update_response = client.chat.completions.create(
        ...
        messages=[
            ...
            },
            {
                "role": "user",
                "content": (
                    "你已经选中了一个最该继续推进的 session。"
                    "请根据该 session 当前状态和前面的路由建议，输出状态更新 JSON。\n\n"
                    "字段要求：\n"
    ...
    save_session_state(selected_session_id, learner_state)

    print("Updated learner state:")
    print(json.dumps(learner_state, ensure_ascii=False, indent=2))
    print()

    print("Selected session persisted to file successfully with atomic write.")
